...
from export_utils import write_markdown, try_make_pdf_from_latex
import time
import logging
...
def run(files):
    ...
    while out_name == '':
        ...
        for per in res["per_file"]:
            ...
            if out_format == 'markdown':
                print('markdown currently not supported!')
                '''
    ...
    print(f'output saved at {out_name}')
    print(f'num tries: {tries}')

    # Export combined
    if out_format == 'markdown':
        print('markdown currently not supported! (2)')
        '''
